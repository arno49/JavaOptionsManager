...
_ids = []  # initialize list of unique ids
...
def convert(obj, ids, attr_type, item_func, cdata, fold_list, parent='root'):
    ...
    logger.info(
        'Inside convert(). obj type is: "%s", obj="%s"' % (type(obj).__name__, unicode_me(obj)),
    )
...
def convert_dict(obj, ids, parent, attr_type, item_func, cdata, fold_list):
    ...
    logger.info(
        'Inside convert_dict(): obj type is: "%s", obj="%s"' % (type(obj).__name__, unicode_me(obj)),
    )
    output = []
    ...
    for key, val in obj.items():
        logger.info(
            'Looping inside convert_dict(): key="%s", val="%s", type(val)="%s"' % (unicode_me(key), unicode_me(val), type(val).__name__),
        )
...
def convert_list(items, ids, parent, attr_type, item_func, cdata, fold_list):
    ...
    for i, item in enumerate(items):
        logger.info(
            'Looping inside convert_list(): item="%s", item_name="%s", type="%s"' % (unicode_me(item), item_name, type(item).__name__),
        )
        attr = {} if not ids else {
            'id': '%s_%s' % (get_unique_id(parent), i + 1)}
...
def convert_kv(key, val, attr_type, attr={}, cdata=False):
    ...
    logger.info(
        'Inside convert_kv(): key="%s", val="%s", type(val) is: "%s"' % (unicode_me(key), unicode_me(val), type(val).__name__),
    )
...
def convert_bool(key, val, attr_type, attr={}):
    ...
    logger.info(
        'Inside convert_bool(): key="%s", val="%s", type(val) is: "%s"' % (unicode_me(key), unicode_me(val), type(val).__name__),
    )
...
def dicttoxml(
    ...
    """
    logger.info(
        'Inside dicttoxml(): type(obj) is: "%s", obj="%s"' % (type(obj).__name__, unicode_me(obj)),
    )
    output = []
    addline = output.append
    if root:
